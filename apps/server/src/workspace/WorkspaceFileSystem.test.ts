...
it.layer(TestLayer, { excludeTestServices: true })("WorkspaceFileSystemLive", (it) => {
  describe("readFile", () => {
    ...
    it.effect("reads files reached through a symlinked workspace directory", () =>
      Effect.gen(function* () {
        const workspaceFileSystem = yield* WorkspaceFileSystem.WorkspaceFileSystem;
        const fileSystem = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const cwd = yield* makeTempDir;
        const linkedDirectory = yield* makeTempDir;
        yield* writeTextFile(linkedDirectory, "docs/guide.md", "# Linked guide\n");
        yield* fileSystem.symlink(linkedDirectory, path.join(cwd, "linked-docs"));

        const result = yield* workspaceFileSystem.readFile({
          cwd,
          relativePath: "linked-docs/docs/guide.md",
        });

        expect(result).toEqual({
          relativePath: "linked-docs/docs/guide.md",
          contents: "# Linked guide\n",
          byteLength: 15,
          truncated: false,
        });
      }),
