...
function normalizeAzureDevOpsCliError(operation: "execute", error: unknown): AzureDevOpsCliError {
  const text = errorText(error);
  const lower = text.toLowerCase();
  if (
    lower.includes("command not found: az") ||
    lower.includes("enoent") ||
    Schema.is(VcsProcessSpawnError)(error)
  ) {
    return new AzureDevOpsCliError({
      operation,
      detail: "Azure CLI `az` with the Azure DevOps extension is required on PATH.",
      cause: error,
    });
  }
  if (
  ...
  return new AzureDevOpsCliError({ operation, detail: text, cause: error });
}
...
const RawAzureDevOpsRepositorySchema = Schema.Struct({
  ...
  project: Schema.optional(Schema.Struct({ name: TrimmedNonEmptyString })),
...
export const make = Effect.fn("makeAzureDevOpsCli")(function* () {
  ...
  return AzureDevOpsCli.of({
    ...
    getDefaultBranch: (input) =>
      executeJson({ cwd: input.cwd, args: ["repos", "show", "--detect", "true"] }).pipe(
        Effect.map((result) => result.stdout.trim()),
        Effect.flatMap((raw) =>
          decodeAzureDevOpsJson(
            raw,
...
export const AzureDevOpsCliLive = Layer.effect(AzureDevOpsCli, make());
