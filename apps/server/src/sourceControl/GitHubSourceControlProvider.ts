...
function toChangeRequestWithUpdatedAt(
  summary: GitHubPullRequests.NormalizedGitHubPullRequestRecord,
): ChangeRequest {
  const { updatedAt: _updatedAt, ...baseSummary } = summary;
  return {
    ...toChangeRequest(baseSummary),
...
export const discovery = {
  ...
  installHint:
    "Install the GitHub command-line tool `gh` via https://cli.github.com/ or your package manager.",
            ...
            return Effect.sync(() => GitHubPullRequests.decodeGitHubPullRequestListJson(raw)).pipe(
              Effect.flatMap((decoded) =>
                Result.isSuccess(decoded)
                  ? Effect.succeed(decoded.success.map(toChangeRequestWithUpdatedAt))
                  : Effect.fail(
