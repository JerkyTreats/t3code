...
function normalizeGitLabCliError(operation: "execute" | "stdout", error: unknown): GitLabCliError {
  if (error instanceof Error) {
    ...
    if (lower.includes("merge request not found") || lower.includes("not found")) {
      return new GitLabCliError({
        operation,
  ...
  return new GitLabCliError({ operation, detail: "GitLab CLI command failed.", cause: error });
}
...
function normalizeHeadSelector(headSelector: string): string {
  return /^[^:]+:(.+)$/.exec(headSelector.trim())?.[1]?.trim() || headSelector.trim();
}
...
function parseRepositoryPath(repository: string): {
  ...
  return {
    namespacePath: parts.length > 1 ? parts.slice(0, -1).join("/") : null,
    projectPath: parts.at(-1) ?? repository.trim(),
  };
}
...
export const make = Effect.fn("makeGitLabCli")(function* () {
  ...
  return GitLabCli.of({
    ...
    getMergeRequest: (input) =>
      execute({ cwd: input.cwd, args: ["mr", "view", input.reference, "--output", "json"] }).pipe(
        Effect.map((result) => result.stdout.trim()),
        Effect.flatMap((raw) =>
    ...
    createMergeRequest: (input) =>
      execute({
        ...
        args: [
          ...
          ...(sourceProjectIdentifier(input.source)
            ? ["--raw-field", `source_project_id=${sourceProjectIdentifier(input.source)}`]
            : []),
          "--raw-field",
          `title=${input.title}`,
          "--field",
    ...
    getDefaultBranch: (input) =>
      execute({ cwd: input.cwd, args: ["api", "projects/:fullpath"] }).pipe(
        Effect.map((result) => result.stdout.trim()),
        Effect.flatMap((raw) =>
          decodeGitLabJson(
            raw,
    ...
    checkoutMergeRequest: (input) =>
      execute({ cwd: input.cwd, args: ["mr", "checkout", input.reference] }).pipe(Effect.asVoid),
  });
});
