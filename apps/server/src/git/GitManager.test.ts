      ...
      getStatus: (input) =>
        Effect.fail(
          new GitHubCli.GitHubCliCommandError({
            ...
            cause: new Error("Unexpected GitHub status lookup"),
          }),
        ),
      login: (input) =>
        Effect.fail(
          new GitHubCli.GitHubCliCommandError({
            command: "gh",
            cwd: input.cwd ?? process.cwd(),
            cause: new Error("Unexpected GitHub login"),
          }),
        ),
      listIssues: (input) =>
        Effect.fail(
          new GitHubCli.GitHubCliCommandError({
            command: "gh",
            cwd: input.cwd ?? process.cwd(),
            cause: new Error("Unexpected GitHub issue list"),
          }),
        ),
      createIssue: (input) =>
        Effect.fail(
          new GitHubCli.GitHubCliCommandError({
            command: "gh",
            cwd: input.cwd ?? process.cwd(),
            cause: new Error("Unexpected GitHub issue create"),
          }),
        ),
      closeIssue: (input) =>
        Effect.fail(
          new GitHubCli.GitHubCliCommandError({
            command: "gh",
            cwd: input.cwd ?? process.cwd(),
            cause: new Error("Unexpected GitHub issue close"),
          }),
        ),
      reopenIssue: (input) =>
        Effect.fail(
          new GitHubCli.GitHubCliCommandError({
            command: "gh",
            cwd: input.cwd ?? process.cwd(),
            cause: new Error("Unexpected GitHub issue reopen"),
          }),
        ),
  ...
  const sourceControlRegistryLayer = Layer.effect(
    ...
    GitHubSourceControlProvider.make.pipe(
      Effect.map((provider) => {
        ...
        const originProvider = SourceControlProvider.SourceControlProvider.of({
          kind: provider.kind,
          listChangeRequests: (request) =>
            provider.listChangeRequests({ ...request, context: originContext }),
          getChangeRequest: (request) =>
            provider.getChangeRequest({ ...request, context: originContext }),
          createChangeRequest: (request) =>
            provider.createChangeRequest({ ...request, context: originContext }),
          getRepositoryCloneUrls: (request) =>
            provider.getRepositoryCloneUrls({ ...request, context: originContext }),
          createRepository: provider.createRepository,
          getDefaultBranch: (request) =>
            provider.getDefaultBranch({ ...request, context: originContext }),
          checkoutChangeRequest: (request) =>
            provider.checkoutChangeRequest({ ...request, context: originContext }),
        });
        return SourceControlProviderRegistry.SourceControlProviderRegistry.of({
          ...
          resolveChangeRequestHandle: () =>
            Effect.succeed({ provider: originProvider, context: originContext }),
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
  ...
  it.effect(
    "rejects PR publication when origin is not configured",
    () =>
      Effect.gen(function* () {
        const repoDir = yield* makeTempDir("t3code-git-manager-");
        yield* initRepo(repoDir);
        ...
        expect(error.message).toContain("Origin-only policy requires an origin remote");
      }),
    12_000,
  );
  ...
  it.effect(
    "rejects fork-backed PR publication when origin is not configured",
    () =>
      Effect.gen(function* () {
        const repoDir = yield* makeTempDir("t3code-git-manager-");
        yield* initRepo(repoDir);
  ...
  it.effect(
    "does not query PR providers before origin-only publication is validated",
    () =>
      Effect.gen(function* () {
        const repoDir = yield* makeTempDir("t3code-git-manager-");
        yield* initRepo(repoDir);
  ...
  it.effect("rejects cross-repo PR publication when origin is not configured", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("t3code-git-manager-");
      yield* initRepo(repoDir);
      const forkDir = yield* createBareRemote();
  ...
  it.effect("prepares pull request threads in local mode by checking out the PR branch", () =>
    Effect.gen(function* () {
      ...
      expect(ghCalls).toContain("pr checkout 64 --repo pingdotgg/codething-mvp --force");
    }),
  );
  ...
  it.effect("rejects cross-repository branch materialization before fallback fetches", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("t3code-git-manager-");
      yield* initRepo(repoDir);
      const originDir = yield* createBareRemote();
      ...
      expect(error.message).toContain("Origin-only policy does not allow materializing");
    }),
  );
  ...
  it.effect("rejects cross-repository worktree pull request threads", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("t3code-git-manager-");
      yield* initRepo(repoDir);
      const originDir = yield* createBareRemote();
  ...
  it.effect("rejects cross-repository local pull request threads", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("t3code-git-manager-");
      yield* initRepo(repoDir);
      const originDir = yield* createBareRemote();
  ...
  it.effect("rejects cross-repository PRs when GitHub omits the repository name", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("t3code-git-manager-");
      yield* initRepo(repoDir);
      const originDir = yield* createBareRemote();
  ...
  it.effect(
    "rejects a cross-repository worktree even when its branch collides with root main",
    () =>
      Effect.gen(function* () {
        const repoDir = yield* makeTempDir("t3code-git-manager-");
        yield* initRepo(repoDir);
  ...
  it.effect("rejects a cross-repository worktree without changing a local main branch", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("t3code-git-manager-");
      yield* initRepo(repoDir);
      const originDir = yield* createBareRemote();
  ...
  it.effect("rejects a cross-repository pull request before reusing its worktree", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("t3code-git-manager-");
      yield* initRepo(repoDir);
      const originDir = yield* createBareRemote();
