/**
 * OpenCodeAdapterLive - OpenCode CLI ACP runtime adapter.
 *
...
const OPEN_CODE_RESUME_VERSION = 1 as const;
...
function parseOpenCodeResume(raw: unknown): { sessionId: string } | undefined {
  ...
  if (raw.schemaVersion !== OPEN_CODE_RESUME_VERSION) return undefined;
...
function makeOpenCodeAdapter(options?: OpenCodeAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const logNative = (
      ...
      method: string,
      payload: unknown,
      _source: "acp.jsonrpc" | "acp.opencode.extension",
    ) =>
      Effect.gen(function* () {
    ...
    const emitPlanUpdate = (
      ...
      rawPayload: unknown,
      source: "acp.jsonrpc" | "acp.opencode.extension",
      method: string,
    ) =>
      Effect.gen(function* () {
    ...
    const stopSessionInternal = (ctx: OpenCodeSessionContext) =>
      Effect.gen(function* () {
    ...
    const startSession: OpenCodeAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          let ctx!: OpenCodeSessionContext;
          ...
          yield* applyRequestedSessionConfiguration({
            ...
            modelSelection: openCodeModelSelection
              ? {
                  model: openCodeModelSelection.model,
                }
              : undefined,
          ...
          const session: ProviderSession = {
            ...
            threadId: input.threadId,
            resumeCursor: {
              schemaVersion: OPEN_CODE_RESUME_VERSION,
...
export function makeOpenCodeAdapterLive(opts?: OpenCodeAdapterLiveOptions) {
  return Layer.effect(OpenCodeAdapter, makeOpenCodeAdapter(opts));
}
