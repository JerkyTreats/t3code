 ...
 * from exact-origin `main` and follow it when it differs, so old releases
 * pick up playbook edits without a release; this copy is the offline fallback.
...
- Repo: https://github.com/JerkyTreats/t3code
...
- Source cache parent (use only after exact source and destination approval): ${input.paths.sourceCacheDir}
`;
