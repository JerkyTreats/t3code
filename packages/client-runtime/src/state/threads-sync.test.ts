...
const makeHarness = Effect.fn("TestEnvironmentThreads.makeHarness")(function* (options?: {
  ...
  const client = {
    ...
    [ORCHESTRATION_WS_METHODS.subscribeThreadV2]: (input: { readonly afterSequence?: number }) =>
      Stream.unwrap(
        Ref.updateAndGet(subscriptionCount, (count) => count + 1).pipe(
          Effect.tap(() => Ref.set(lastSubscribeAfterSequence, input.afterSequence)),
          Effect.tap(() =>
...
describe("EnvironmentThreads", () => {
  ...
  it.effect("resumes a warm cache through the selected stream without an HTTP fetch", () =>
    Effect.gen(function* () {
      const harness = yield* makeHarness({ cached: BASE_THREAD, threadSyncV2: true });

      yield* Queue.offer(
        harness.v2Inputs,
        v2TitleUpdated("Live title", CACHED_SNAPSHOT_SEQUENCE + 1),
      );
      ...
      expect(yield* Ref.get(harness.subscriptionVersions)).toEqual(["v2"]);
    }),
  );

  it.effect("seeds from an atomic HTTP snapshot before selecting V2", () =>
    Effect.gen(function* () {
      const httpThread = { ...BASE_THREAD, title: "HTTP title" };
      const harness = yield* makeHarness({
        httpSnapshot: Option.some({ snapshotSequence: 3, thread: httpThread }),
        threadSyncV2: true,
      });

      yield* Queue.offer(harness.v2Inputs, v2TitleUpdated("Live title", 4));
      const state = yield* awaitThreadState(
        harness.observed,
        (value) =>
          value.status === "live" &&
    ...
    Effect.gen(function* () {
      ...
      const live = yield* awaitThreadState(
        ...
        (state) =>
          ...
          state.data.value.messages.length === 2,
      );
  ...
  it.effect("rejects incomplete hydration responses before publishing deferred payloads", () =>
    Effect.gen(function* () {
      const activity: OrchestrationThreadActivity = {
        id: EventId.make("activity-requested"),
        tone: "tool",
        kind: "tool.call",
        summary: "Deferred activity",
        payload: {
          __t3Deferred: "thread-activity-payload",
          byteLength: 4097,
        },
      ...
      const harness = yield* makeHarness({
        ...
        hydrate: () =>
          Effect.succeed({
            payloads: [
              {
                activityId: EventId.make("activity-unrequested"),
                payload: { leaked: true },
                byteLength: 16,
              },
            ],
      ...
      yield* Queue.offer(harness.v2Inputs, v2Snapshot({ ...BASE_THREAD, activities: [activity] }));
      const failed = yield* awaitThreadState(
        harness.observed,
        (state) => state.syncStatus?.phase === "error",
      );

      expect(Option.isNone(failed.data)).toBe(true);
      expect(failed.syncStatus).toMatchObject({
        phase: "error",
        version: "v2",
      });
      expect(yield* Ref.get(harness.v2SubscriptionCount)).toBe(1);
  ...
  it.effect("preserves data after a domain failure and resumes on a replacement session", () =>
    Effect.gen(function* () {
      ...
      const subscriptionsBeforeReplacement = yield* Ref.get(harness.subscriptionCount);
      yield* harness.replaceSession;
      for (let attempt = 0; attempt < 100; attempt += 1) {
        if ((yield* Ref.get(harness.subscriptionCount)) > subscriptionsBeforeReplacement) {
          break;
        }
        yield* Effect.yieldNow;
      }
      ...
      const retrying = yield* Ref.get(harness.latest);
      ...
      expect(yield* Ref.get(harness.subscriptionCount)).toBeGreaterThan(
        subscriptionsBeforeReplacement,
      );

      const subscriptionsAfterRecovery = yield* Ref.get(harness.subscriptionCount);
      yield* harness.replaceSession;
      for (let attempt = 0; attempt < 100; attempt += 1) {
        if ((yield* Ref.get(harness.subscriptionCount)) > subscriptionsAfterRecovery) {
          break;
        }
        yield* Effect.yieldNow;
      }
      const stable = yield* Ref.get(harness.latest);
      expect(Option.getOrThrow(stable.data).title).toBe("Recovered thread");
      expect(Option.isNone(stable.error)).toBe(true);
      expect(stable.syncStatus).toMatchObject({
        phase: "subscribing",
        version: "v1",
        error: null,
      });
