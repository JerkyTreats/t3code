...
[F29 local voice output](fork/F29-local-voice-output.md) owns opt-in, device-local spoken replies and per-device selection from a bounded Pocket TTS voice catalog. Turn-only response guidance feeds the existing provider boundary, and an authenticated bounded proxy sends the completed reply verbatim with the selected named voice to an operator-configured local Pocket TTS service. Shared admission and HTTP owners prevent historical or cross-device autoplay while preserving a locally submitted turn through same-thread draft promotion; web and desktop Settings add an explicit voice preview, mobile Settings exposes the same catalog, and every audio owner retains stop, replay and failure states. Coding-model serving and existing dictation remain unchanged.
